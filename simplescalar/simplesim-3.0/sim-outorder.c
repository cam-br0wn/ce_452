...
/* Number of cycles waiting for the bus */
static counter_t sim_waited_for_bus = 0;
...
tick_t bus_available = 0;
#define BUS_ACCESS_DELAY 1
#define BUS_WIDTH 8

unsigned int get_bus (tick_t now, int bsize)
{
  unsigned int latency = (bsize / BUS_WIDTH) + BUS_ACCESS_DELAY;

  if ((unsigned int)bus_available <= (unsigned int)now)
    {
      bus_available = now + latency;
      return 0;
    }
  else
    {
      unsigned int returnval = (unsigned int)bus_available - (unsigned int)now;
      sim_waited_for_bus += returnval;
      bus_available += latency;
      return returnval;
    }
}
  ...
  if (cache_dl2)
    {
      ...
      lat = get_bus (now, bsize) + cache_access(cache_dl2, cmd, baddr, NULL, bsize,
...
if (cache_il2)
    {
      ...
      lat = get_bus (now, bsize) + cache_access(cache_il2, cmd, baddr, NULL, bsize,
						/* now */now, /* pudata */NULL, /* repl addr */NULL);
      if (cmd == Read)
	return lat;
      else
...
sim_reg_stats(struct stat_sdb_t *sdb)   /* stats database */
{
  ...
  /* L1/L2 bus stats */
  stat_reg_counter(sdb, "sim_waited_for_bus",
		   "total simulation cycles for the bus",
		   &sim_waited_for_bus, /* initial value */0, /* format */NULL);
...
#endif /* HOST_HAS_QWORD */
      enum md_fault_type fault;
      long long internal_count = fastfwd_count * 1000000;

      fprintf(stderr, "sim: ** fast forwarding %d (x1000000) insts **\n", fastfwd_count);

      for (icount=0; icount < internal_count; icount++)
	{
